...
import com.viit.steganography.util.AudioFileHelper;
import com.viit.steganography.util.PBEncryptionHelper;
...
public class SteganographyHandler {
	...
	public boolean embed() {
		...
		try {
			encryptedTextBytes = PBEncryptionHelper.encrypt(textDataBytes,password);
			if (encryptedTextBytes.length * 8 > audioBytes.length)
				return false;

		} catch (Exception e) {
			System.out.println("Error while encrypting the plain text data: "+ e);
		}
		System.out.println("The encrypted message length: "+ encryptedTextBytes.length);
