package com.viit.steganography.ui;

import com.viit.steganography.ExtractionHandler;
...
public class ExtractAction implements WizardButtonListener {
	...
	public EnterPasswordExtract ep;
	...
	public ExtractAction(MainWindow m) {
		...
		ep = new EnterPasswordExtract("3. Enter password");
		wf.addWizPanel(ep);

		vo = new VerifyOptions("4. Verify Extraction Options");
		wf.addWizPanel(vo);
	...
	public void buttonClicked(WizardButtonEvent wbe) {
		if (wbe.getButtonType() == WizardButtonListener.NEXT
			...
			ExtractionHandler.startExtract(options);
			...
			try {
				java.io.BufferedReader br = new java.io.BufferedReader(
						new java.io.FileReader(options.getOutputFile()));
				String line = null;
				while ((line = br.readLine()) != null)
					sef.addOutputLine(line);
				br.close();
				sef.setDisplayText("Output File : "
